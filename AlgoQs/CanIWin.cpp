...
//            v.emplace_back(i);
//
//        vector<vector<pair<int, int>>> dp(maxChoosableInteger, vector<pair<int, int>>(maxChoosableInteger, { 0,0 }));
//        int runningTotal = 0;
//
...
//                        continue;
//                    }
//                    if (i + 1 < maxChoosableInteger &&
//                        i < maxChoosableInteger &&
//                        j < maxChoosableInteger &&
//                        j - 1 >= 0)
//                    {
//                        dp[i][j].first = max(v[i] + dp[i + 1][j].second,
//                            v[j] + dp[i][j - 1].second);
//
//                        if (v[j] + dp[i][j - 1].second > v[i] + dp[i + 1][j].second)
//                        {
...
//                        }
//
//                        if (dp[0][maxChoosableInteger - 1].first >= desiredTotal)
//                            return true;
//                    }
//                }
//            }
...
public:
    vector<int> largestDivisibleSubset(vector<int>& nums) {
        ...
        possibleSubsets.push_back({ nums[0] });

        for (int i = 1;i < nums.size();++i)
        {
            int j = i - 1;
            possibleSubsets.emplace_back();
            while (j >= 0)
            {
                bool res = std::all_of(possibleSubsets[j].begin(), possibleSubsets[j].end(),
                    [&](int& num) {
                    return num % nums[i] == 0 || nums[i] % num == 0;
                });
                if (res && possibleSubsets[j].size() + 1 > possibleSubsets[i].size())
                {
                    possibleSubsets[i] = possibleSubsets[j];
                    possibleSubsets[i].emplace_back(nums[i]);
                }
                --j;
            }
        }

        return *std::max_element(possibleSubsets.begin(), possibleSubsets.end(), [](auto set1, auto set2) {
...
//    auto vec = s.largestDivisibleSubset(nums);
//    return 0;
//}
...
public:
    int deleteAndEarn(vector<int>& nums) {
        ...
        dp[0] = nums[0] * umap[nums[0]];
        int res = 0;
        for (int i = 1;i < nums.size();++i)
        {
            if (nums[i] != nums[i - 1] + 1)
            {
                dp[i] = max(dp[i], dp[i - 1] + nums[i] * umap[nums[i]]);
            }
            else
            {
                if (i >= 2)
                    dp[i] = max(dp[i], dp[i - 2] + nums[i] * umap[nums[i]]);
                else
                    dp[i] = max(dp[i], nums[i] * umap[nums[i]]);
            }
            res = max(res, dp[i]);
        }
