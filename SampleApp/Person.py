...
def print_friend_info(person):
    ...
    print("Age : " + str(person._age))
    if(person._friend is not None):
        print("Friends with "+ person._friend.get_name())

def create_fy():
    p = Person("Philip J.Fry",25,'M')
    return p

def make_friends(person_one,person_two):
    person_one._friend = person_two
    person_two._friend = person_two

p1 = Person("john",25,'M')
p2 = Person("Mary",24,'F')
make_friends(p1,p2)
print_friend_info(p1)
